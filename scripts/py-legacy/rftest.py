...
if __name__ == "__main__":
    ...
    parser.add_argument('address', type=str, nargs='?', default='00001', help="Address to listen to (3 to 5 ASCII characters).")
    ...
    nrf = NRF24(pi, ce=8, payload_size=psize, channel=CSN, data_rate=RF24_DATA_RATE.RATE_250KBPS, pa_level=RF24_PA.MIN)
    ...
    try:
        ...
        while True:
            ...
            while nrf.data_ready():
                ...
                # hex = [f'{i:02x}' for i in payload]
                print(hex)

                # Show message received as hex.
                # print(f"{now:%Y-%m-%d %H:%M:%S.%f}: pipe: {pipe}, len: {len(payload)}, bytes: {hex}, count: {count}")
                # temperature = int.from_bytes(payload, byteorder="little", signed=True)
